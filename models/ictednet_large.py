...
def _extract_compatible_pretrained_state(pretrained_path, model_state):
    checkpoint = torch.load(pretrained_path, map_location='cpu')
    state_dict = checkpoint['state_dict'] if isinstance(checkpoint, dict) and 'state_dict' in checkpoint else checkpoint
    if not isinstance(state_dict, dict):
        raise TypeError('Unsupported checkpoint format: expected dict or dict["state_dict"].')
    ...
    for key, value in state_dict.items():
        ...
        if key.startswith('model.'):
            candidate_keys.append(key[len('model.'):])
        if key.startswith('model.module.'):
            candidate_keys.append(key[len('model.module.'):])
        for cand in candidate_keys:
            if cand in model_state and model_state[cand].shape == value.shape:
                compatible_state[cand] = value
                break
...
def get_seg_model(cfg, imgnet_pretrained):
    ...
    pretrained_path = str(cfg.MODEL.PRETRAINED).strip()
    if pretrained_path:
        model_state = model.state_dict()
        pretrained_state = _extract_compatible_pretrained_state(pretrained_path, model_state)
        model_state.update(pretrained_state)
        model.load_state_dict(model_state, strict=False)
        logging.info('Loaded %d compatible pretrained parameters from %s', len(pretrained_state), pretrained_path)
        if imgnet_pretrained:
            logging.info('ImageNet pretrained mode is enabled.')
    else:
        logging.info('MODEL.PRETRAINED is empty. Training from scratch.')
